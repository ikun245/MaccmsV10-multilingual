var debug = false; // 调试时设为 true，线上设为 false
...
var MacPlayer = {
    'GetDate': function(f,t){
        ...
        f = f.replace(/MM/, (t.getMonth()+1) > 9 ? (t.getMonth()+1) : '0'+(t.getMonth()+1));
        f = f.replace(/M/g, t.getMonth()+1);
        f = f.replace(/w|W/g, Week[t.getDay()]);
        f = f.replace(/dd|DD/, t.getDate() > 9 ? t.getDate() : '0'+t.getDate());
        f = f.replace(/d|D/g, t.getDate());
        f = f.replace(/hh|HH/, t.getHours() > 9 ? t.getHours() : '0'+t.getHours());
        f = f.replace(/h|H/g, t.getHours());
        f = f.replace(/mm/, t.getMinutes() > 9 ? t.getMinutes() : '0'+t.getMinutes());
        f = f.replace(/m/g, t.getMinutes());
        f = f.replace(/ss|SS/, t.getSeconds() > 9 ? t.getSeconds() : '0'+t.getSeconds());
    ...
    'Play': function(){
        document.write('<style>.MacPlayer{background:#000000;font-size:14px;color:#F6F6F6;margin:0;padding:0;position:relative;overflow:hidden;width:' + this.Width + ';height:' + this.Height + ';min-height:100px;}.MacPlayer table{width:100%;height:100%;}.MacPlayer #playleft{position:inherit!important;width:100%;height:100%;}</style><div class="MacPlayer">' +
        ...
        '<table border="0" cellpadding="0" cellspacing="0"><tr><td id="playleft" valign="top">&nbsp;</td></tr></table></div>');
