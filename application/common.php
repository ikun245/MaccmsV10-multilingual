...
function get_multi_ad($type) {
    $config = \think\Config::get('adv_multi');
    if (empty($config)) {
        $file = APP_PATH . 'extra/adv_multi.php';
        if (is_file($file)) {
            $config = include $file;
        }
    }
    ...
    if ($type == 'popup') {
        if (!empty($config['popup']['status']) && !empty($config['popup']['lang'][$lang])) {
            return $config['popup']['lang'][$lang];
        }
        return null;
    }
    
    if (!empty($config[$type])) {
        foreach ($config[$type] as $item) {
            if (!empty($item['status']) && !empty($item['lang'][$lang]) && !empty($item['lang'][$lang]['img'])) {
                $ads[] = $item['lang'][$lang];
            }
        }
    }
